...
class MailGunAPI(object):
    def __init__(self, config):
        ...
        self.api_url = config.get("MAILGUN_API_URL", MAILGUN_API_URL)
    ...
    def list_routes(self):
        request = requests.get(self.routepoint, auth=self.auth)
        if not request.ok:
            raise MailGunException(
                "Failed to get routes. Please check your configuration e.g. your mailgun key."
            )
    ...
    def create_route(self, recipient="user", dest="/messages/", priority=0):
        ...
        if self.route_exists(route):
            ...
            return requests.post(self.routepoint, auth=self.auth, data=route)
    ...
    def verify_email(self, email):
        ...
        if timestamp is None or token is None or signature is None:
            raise MailGunException(
                "Mailbox Error: credential verification failed.",
                "Not enough parameters",
            )

        message = "{}{}".format(timestamp, token).encode("utf-8")
        signature_calc = hmac.new(
            key=self.api_key, msg=message, digestmod=hashlib.sha256
        ).hexdigest()
